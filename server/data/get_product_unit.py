...
# def Units():
dfUnits = ProductDetail.objects.raw("""  
        SELECT *
        FROM units
        WHERE units.deleted_at IS NULL             
        """)
dfUnits = pd.DataFrame([item.__dict__ for item in dfUnits]).drop(['_state'], axis=1)
print(dfUnits)
dfUnits.to_csv('../data_csv/Units.csv', index= False)
