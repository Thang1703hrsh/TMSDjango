...
#         SELECT 1 id, POD.total_amount , POD.purchase_order_id , POD.is_completed, POS.status as po_status
...
#         SELECT 1 id, PD.product_id, PL.product_unit_id ,PL.quantity AS purchase_order_quantity, PL.purchase_order_id, PL.is_complete 
...
# # enter the inventory of the purchase order
# dfStockInPurchaseOrder= ProductDetail.objects.raw("""  
#         SELECT 1 id, PSIRT.product_id , PSIRT.purchase_order_id ,SUM(PSIRT.quantity) AS quantity_arrived
#                 FROM product_stock_in_real_details AS PSIRT 

#                 WHERE PSIRT.deleted_at IS NULL
#                     AND purchase_order_id IS NOT NULL

#                 GROUP BY PSIRT.product_id, PSIRT.product_unit_id, PSIRT.purchase_order_id 
